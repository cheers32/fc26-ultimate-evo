...
export function applyEvo(
  ...
  const currentOvr = Math.max(state.ovr, Math.min(state.ovr + evo.ovrBoost.boost, evo.ovrBoost.limit));
    ...
    // Every plain PlayStyle on the card counts, the player's own picks included.
    //
    // For a while picks were exempted, on the strength of a single van Dijk chain that looked like
    // it had been granted a PlayStyle it had no room for. That reading did not hold up, and the
    // exemption was wrong three times over: it let an evo hand out PlayStyle+ past its cap, plain
    ...
    // Counter Culture takes a four-slot card to five, and the fifth slot stays a slot: it is what
    // the card holds afterwards, and what the picker may fill if the evo leaves it empty.
    //
    // Gold only. On the plain side the number an evo prints is a ceiling on its own grants, not a
    // promise to enlarge the card: The Perfect Break says 8 and a card with seven plain slots comes
    // out of it with seven, because there was nowhere for the second grant to go. Raising both
    // handed that card a Trickster it does not get in game.
    ...
    batch.gold.forEach((ps) => {
      ...
      let upgraded = false;
      if (!hasGold) {
        // Every PlayStyle+ on the card counts, free picks included, exactly as on the plain side.
