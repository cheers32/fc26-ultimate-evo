import { EvolutionDefinition } from '../../types/player';

/**
 * One of the four "Max" evos — passing straight to 99 and nothing else.
 *
 * FUTBIN prints the boost as +98 with no cap, which is the point: it is not a boost so much as an
 * instruction to fill the face. That makes it worth exactly what the card is short of 99 there, so
 * it belongs on a card with a real hole rather than one already in the high 90s — and it belongs
 * last in a chain, since it will be worth less after anything else has raised the same face.
 */
export const maxPassing1237: EvolutionDefinition = {
  id: '1237',
  name: 'Max Passing',
  futbinLink: 'https://www.futbin.com/26/evolutions/1237/max-passing',
  version: 'FC 26',
  description: 'Boost your player to 99 Passing. Season 10 level 27 reward.',
  descriptionZh: "把球员的传球提升到 99。赛季 10 等级 27 奖励。｜适合：非门将，传球面板填满，中场首选，应放链路最后。",
  cost: 'Season 10 — Level 27 Reward',
  requirements: {
    maxOvr: 98,
    excludedPositions: ['GK']
  },
  ovrBoost: { boost: 2, limit: 99 },
  faceBoosts: {
    pas: { boost: 98, limit: 99 }
  },
  subStatBoosts: {},
  playStylesAdded: { gold: [], silver: [] },
  levels: [
    { name: 'Level 1', upgrades: ['OVR +2', 'Passing +98'] }
  ],
  maxRepeatable: 1
};
